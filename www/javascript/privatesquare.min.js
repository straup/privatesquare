...
function privatesquare_init(a){a||(a="foursquare");$("body").attr("data-privatesquare-provider",a);if(!window.navigator.onLine&&_cfg.offline_appcache){privatesquare_deferred_checkin(null,null,"offline");$("#navi-online").hide();$("#navi-offline").show()}else{$("#navi-online").show();$("#navi-offline").hide();privatesquare_whereami(function(b){var c=b.coords.latitude;b=b.coords.longitude;var d=$("#venues");d.attr("data-geolocation-latitude",c);d.attr("data-geolocation-longitude",b);if(a=="stateofmind")privatesquare_stateofmind_fetch_venues(c,
b);else a=="nypl"?privatesquare_nypl_fetch_venues(c,b):privatesquare_foursquare_fetch_venues(c,b)},_privatesquare_geolocation_onerror);$("#checkin").submit(privatesquare_submit);$("#again").click(privatesquare_reset);privatesquare_set_status("Asking the sky where you are...")}}function _privatesquare_geolocation_onerror(){privatesquare_set_status("Huh. I have no idea where you are...")}
function privatesquare_reset(){$("#venues").hide();privatesquare_unset_status();_privatesquare_hide_map();privatesquare_init()}function privatesquare_submit(){var a=privatesquare_gather_args();if(a.venue_id==-1){privatesquare_search();return false}privatesquare_checkin(a,privatesquare_checkin_onsuccess);$("#venues").hide();_privatesquare_hide_map();privatesquare_set_status("Checking in...");return false}
...
function privatesquare_search(){if(searching)return false;searching=true;_privatesquare_hide_map();$("#venues").hide();var a=prompt("Search for a particular place");if(!a){var b=$("#venues").attr("data-venues-provider");privatesquare_set_status('Okay, I\'m giving up. <a href="#" onclick="privatesquare_init();return false;">Start over</a> if you want change your mind.');return false}b=privatesquare_venues_provider();var c=null;if(b=="foursquare")c=function(d){privatesquare_foursquare_fetch_venues(d.coords.latitude,
...
function privatesquare_whereami(a,b){try{navigator.geolocation.getCurrentPosition(a,b,{enableHighAccuracy:true,maximumAge:1E3})}catch(c){alert("The sky is angry offering only this, today: "+c)}}function privatesquare_abs_root_url(){return document.body.getAttribute("data-abs-root-url")}function privatesquare_venues_provider(){return $("body").attr("data-privatesquare-provider")};function privatesquare_foursquare_fetch_venues(a,b,c){a={latitude:a,longitude:b};if(c!="")a.query=c;privatesquare_api_call("foursquare.venues.search",a,_privatesquare_foursquare_fetch_venues_onsuccess);privatesquare_set_status("Fetching nearby places...")}
...
b=$("#where");b.attr("data-crumb",a.crumb);b.html(c);b.change(_privatesquare_where_onchange);$("#what").change(_privatesquare_what_onchange);_privatesquare_show_map(a.latitude,a.longitude);privatesquare_unset_status();$("#venues").show();$("#checkin").submit(privatesquare_submit)}else{c="You appear to have fallen in to a black hole. There's nothing around here";if(a.query)c+=" that looks like <q>"+htmlspecialchars(a.query)+"</q>";c+='. <a href="#" onclick="privatesquare_search();return false;">Try again</a>';
c+=' or <a href="#" onclick="privatesquare_init();return false;">start from scratch</a>?';privatesquare_set_status(c)}}};function privatesquare_nypl_fetch_venues(a,b,c){$("#broadcast").attr("disabled","disabled");$("#venues").attr("data-venues-provider","nypl");a={latitude:a,longitude:b};if(c!="")a.query=c;privatesquare_api_call("nypl.venues.search",a,_privatesquare_nypl_fetch_venues_onsuccess);privatesquare_set_status("Asking the NYPL for buildings nearby...")}
function _privatesquare_nypl_fetch_venues_onsuccess(a){privatesquare_unset_status();if(a.stat=="ok"){var b=a.venues.length;if(b){for(var c="",d=0;d<b;d++){var e=a.venues[d];c+='<option value="'+e.id+'">'+e.name+"</option>"}c+='<option value="-1">\u2013\u2013 none of the above / search \u2013\u2013</option>';b=$("#where");b.attr("data-crumb",a.crumb);b.html(c);b.change(_privatesquare_where_onchange);$("#what").change(_privatesquare_what_onchange);_privatesquare_show_map(a.latitude,a.longitude);privatesquare_unset_status();
